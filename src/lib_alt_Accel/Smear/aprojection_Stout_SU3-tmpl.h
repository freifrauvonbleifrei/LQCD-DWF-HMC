...
void AProjection_Stout_SU3<AFIELD>::init()
{
  ...
  m_U.reset(m_Nst, 1);    // hoge
  m_Cst.reset(m_Nst, 1);
  m_Uorg.reset(m_Nst, 1);
}
                                            ...
                                            const AFIELD& Uorg)
{
  ...
  vout.detailed(m_vl, "%s: project with AFIELD is not yet ready\n",
                class_name.c_str());

  AIndex_lex<real_t,AFIELD::IMPL> index_lex;

#pragma omp barrier
  reverse_gauge(index_lex, m_Cst, Cst);
  reverse_gauge(index_lex, m_Uorg, Uorg);
  project(m_U, alpha, m_Cst, m_Uorg);
  convert_gauge(index_lex, U, m_U);
#pragma omp barrier
}
