...
#all: msg version_h lib program-with-lib all-done
all: lib


## build library
#lib: msg check-extra_libs link-inline make-inc
lib: msg version_h check-extra_libs link-inline make-inc
	$(MAKE) -f src/Makefile build_dir=$(build_dir) lib

## build test application
#program: msg make-test_dir
program: msg version_h make-test_dir
...
program-with-lib: msg version_h lib make-test_dir
	build_dir_path=`cd $(build_dir) && pwd`; \
	test_dir_path=`cd $(test_dir) && pwd`; \
	cd tests && $(MAKE) bridge_install_path=$$build_dir_path test_dir=$$test_dir_path test="""$(test)""" test_flags="""$(CPPFLAGS_TEST)"""

## derive version.h from VERSION at the top directory
version_h: src/version.h VERSION

src/version.h: VERSION
	@echo "#define BRIDGE_VERSION \""`cat VERSION`"\"" > src/version.h
...
.PHONY: doxygen
doxygen:
	@if [ -e ./docs/doxygen.conf.in ]; then \
	  echo "generating doxygen documents ..."; \
	  ( \
	    cd ./docs; \
	    sed -e "s/@@VERSION@@/"`cat ../VERSION`"/" doxygen.conf.in > doxygen.conf; \
